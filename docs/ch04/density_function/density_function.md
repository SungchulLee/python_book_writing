...
조건 1 ($f \geq 0$): $0 \leq x \leq 2$ 에서 $x(2-x) \geq 0$ 이므로 $c \geq 0$.
...
따라서 $f(x) = \dfrac{3}{4}\,x(2-x)$. 검증: $f(1) = 3/4 > 0$, $f(0) = f(2) = 0$, 면적 $= 1$.

이때 $P(0 \leq X \leq 1) = \displaystyle\int_0^1 \dfrac{3}{4}x(2-x)\,dx = \dfrac{3}{4}\cdot(1 - 1/3) = \dfrac{1}{2}$. 곡선이 $x = 1$ 을 중심으로 대칭이므로 절반의 확률이 자연스럽다.

!!! info "보기 1의 교훈"
    - **미정 상수의 결정**은 항상 조건 2 (적분 = 1) 에서 시작한다.
    - 미정 상수를 결정한 뒤 조건 1 (부호) 도 확인해야 함 — 음수 상수가 나오면 함수가 밀도함수가 아니다.
...
**단계 1: 연속성.** $f$ 가 $x = 1$ 에서 자연스럽게 정의되려면 두 식의 값이 같아야 한다 (일반적으로 밀도함수는 가산 측도 영집합에서의 정의 차이를 허용하지만, 문제에서 piecewise 정의가 명확히 주어지므로 연속을 가정한다).

$x = 1$ 에서: 좌측 $a(b + 1)$, 우측 $-a(b+1)(1 - 2) = a(b+1)$. **자동 일치**.

**단계 2: 적분 = 1.**

$$
\int_0^1 a(bx + 1)\,dx + \int_1^2 -a(b+1)(x-2)\,dx = a\!\left(\frac{b}{2} + 1\right) + \frac{a(b+1)}{2} = a(b + 1) + \frac{ab}{2} - \frac{ab}{2} = a(b+1)
$$

잠시 — 다시 정확히 전개. $\int_0^1 (bx+1)dx = b/2 + 1$. $\int_1^2 (x-2)dx = [x^2/2 - 2x]_1^2 = (2 - 4) - (1/2 - 2) = -2 + 3/2 = -1/2$. 따라서

$$
a(b/2 + 1) + (-a(b+1))(-1/2) = a(b/2 + 1) + a(b+1)/2 = \frac{a\,b}{2} + a + \frac{ab + a}{2} = ab + \frac{3a}{2} = 1
$$

전개의 마지막을 정리:

$$
a\bigl(b/2 + 1 + (b+1)/2\bigr) = a\bigl(b/2 + b/2 + 1 + 1/2\bigr) = a\bigl(b + 3/2\bigr) = 1
$$

따라서

$$
a = \frac{1}{b + 3/2} = \frac{2}{2b + 3}\quad (b \geq 0)
$$
...
$b \to 0$: $c \to 15/18 = 5/6$. $b \to \infty$: $c \to 14/(12) = 7/6$. 그리고 $b \geq 0$ 범위에서 $c$ 는 단조증가 (분자 미분 $14$ 와 분모 미분 $12$ 의 비교: $\dfrac{dc}{db} = \dfrac{14\cdot 6(2b+3) - 12(14b+15)\cdot 1}{[6(2b+3)]^2} = \dfrac{84(2b+3) - 12(14b+15)}{\cdots} = \dfrac{168b + 252 - 168b - 180}{\cdots} = \dfrac{72}{\cdots} > 0$).

따라서 $\displaystyle\frac{5}{6} \leq c < \frac{7}{6}$. $\quad\square$
